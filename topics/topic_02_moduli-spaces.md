...
> Our research advances the (local) Langlands Programme through the arithmetic
> properties of associated moduli spaces and representation categories. More
...
> scalar curvature metrics), and mathematical physics (moduli of stable curves and
...
1. **Construction and existence** (Shimura varieties, $G$-shtukas, Galois deformation spaces).
2. **Compactification and stratifications**.
3. **Invariants and cohomology** (intersection theory, derived categories, topological recursion).
...
force one to work with **algebraic stacks** (e.g. the Deligne–Mumford stack
...
\mathcal{T}_g/\mathrm{MCG}(\Sigma_g)$ as a quotient of Teichmüller space by the
mapping class group, linking it to the classifying space $B\,\mathrm{Diff}(\Sigma_g)$
...
### Level 2 — Master
MM studies the **local** Langlands correspondence and its geometrisation. Moduli
spaces enter as **Shimura varieties** and **moduli of (local/global)
...
Emerton–Gee–Hellmann) phrases it as an equivalence of derived categories of
...
Kontsevich model. On the arithmetic side, the conjectural Langlands correspondence
is refined to an equivalence of **derived categories of (constructible/coherent)
...
$\mathcal{R}^+(M)$ of positive scalar curvature metrics. References:
...
Weiss embedding calculus**, **surgery**, and **pseudoisotopy/Waldhausen K-theory**;
...
**positive scalar curvature cobordism category** and index-theoretic obstructions
